...
function solve() {
    ...
    # for i in $(seq 1 $TIMEFRAMELIMIT); do
        ...
        time -p (resourcelimit -t "$TIMELIMIT" -m "$MEMLIMIT" "$ABC" -c "ssat" "$CASEFILE") >"$LOGFILE" 2>&1
...
function collectInfo() {
    ...
    result=$(extractInfo "  > Satisfying probability:" "$LOGFILE")
    if [ "$result" != "" ]; then
        echo -n ",$result,$result" >>"$INFO_COLLECT_FILE"
    else
        
        result=$(extractInfo "  > Best upper bound:" "$LOGFILE")
        echo -n ",$result" >>"$INFO_COLLECT_FILE"
        result=$(extractInfo "  > Best lower bound:" "$LOGFILE")
        echo -n ",$result" >>"$INFO_COLLECT_FILE"
