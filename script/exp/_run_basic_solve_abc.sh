...
function solve() {
    ...
    # for i in $(seq 1 $TIMEFRAMELIMIT); do
        ...
        time -p (resourcelimit -t "$TIMELIMIT" -m "$MEMLIMIT" "$ABC" -q "ssat -v $CASEFILE") >"$LOGFILE" 2>&1
...
function collectInfo() {
    ...
    result=$(extractInfo "  > Upper bound =" "$LOGFILE")
    if [ "$result" != "" ]; then
        echo -n ",$result" >>"$INFO_COLLECT_FILE"
    else
...
# result collection
echo "case,result,ratio,time,pretime" >"$INFO_COLLECT_FILE"
